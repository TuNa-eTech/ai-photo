...
export class BearerAuthGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    const auth = (req.headers['authorization'] ||
      (req.headers['Authorization'] as any)) as string | undefined;
    if (!auth || !auth.startsWith('Bearer ')) {
      this.logger.warn(
        `Missing or invalid Authorization header for ${req.url}`,
      );
    ...
    try {
      ...
      this.logger.error(
        `Firebase token verification failed for ${req.url}: ${errorMessage}`,
      );
      throw new UnauthorizedException({
        code: 'unauthorized',
