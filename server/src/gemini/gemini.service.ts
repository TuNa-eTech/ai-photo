...
export class GeminiService {
  ...
  constructor(private readonly configService: ConfigService) {
    ...
    this.timeoutMs =
      this.configService.get<number>('gemini.timeoutMs') || 45000;
    ...
    if (this.apiKey) {
      ...
      this.logger.warn(
        'GEMINI_API_KEY is not configured. Image processing will be disabled.',
      );
    }
  }
  ...
  async generateImage(dto: GenerateImageDto): Promise<GeminiResponse> {
    ...
    try {
      ...
      const base64Data = dto.imageBase64.replace(
      ...
      const generationTime = Date.now() - startTime;
      this.logger.error(
        `Image generation failed after ${generationTime}ms:`,
        error.message,
      );
      ...
      if (
        ...
        const retryAfter = error.error?.details?.find(
          (d: any) => d['@type'] === 'type.googleapis.com/google.rpc.RetryInfo',
        )?.retryDelay;
        const message = error.error?.message || 'API quota exceeded';
        throw new GeminiAPIException(
          `Quota exceeded. Please check your billing and retry later. ${retryAfter ? `Retry after: ${retryAfter}` : ''}`,
          429,
        );
      }
